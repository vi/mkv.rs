...
macro_rules! elements_database {
    ($($cl:ident $t:ident $id:expr),*,) => {
        ...
        impl ClassName {
            pub fn from_id(id: EbmlElementRawId) -> Self {
                use self::ClassName::*;
            ...
            pub fn to_id(self) -> EbmlElementRawId {
                use self::ClassName::*;
            ...
            pub fn get_type(self) -> super::Type {
                use self::ClassName::*;
...
#[test]
fn eldb_test_id_to_class() {
    assert_eq!(ClassName::from_id(0x1A45DFA3), ClassName::EBML);
    assert_eq!(ClassName::from_id(0x002EB524), ClassName::ColourSpace);
    assert_eq!(ClassName::from_id(0x000000EC), ClassName::Void);
    assert_eq!(ClassName::from_id(0x00004285),
               ClassName::DocTypeReadVersion);
    assert_eq!(ClassName::from_id(0x1A45DBBBB), ClassName::Unknown);
}

#[test]
fn eldb_test_class_to_id() {
    assert_eq!((ClassName::EBML).to_id(), 0x1A45DFA3);
    assert_eq!((ClassName::ColourSpace).to_id(), 0x002EB524);
    assert_eq!((ClassName::Void).to_id(), 0x000000EC);
    assert_eq!((ClassName::DocTypeReadVersion).to_id(), 0x00004285);
}

#[test]
fn eldb_test_class_to_type() {
    use super::Type::*;
    assert_eq!((ClassName::EBML).get_type(), Master);
    assert_eq!((ClassName::ColourSpace).get_type(), Binary);
    assert_eq!((ClassName::Void).get_type(), Binary);
    assert_eq!((ClassName::DocTypeReadVersion).get_type(), Unsigned);
}
